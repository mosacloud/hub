...
test.describe('Home page', () => {
  ...
  test('checks all the elements are visible', async ({ page }) => {
    ...
    await expect(
      page.getByText(
        'LaSuite Hub is the entry point to La Suite collaborative tools.',
      ),
    ).toBeVisible();
    await expect(
      page.getByRole('link', {
    ...
    await expect(
      page.getByText('You can easily self-host LaSuite Hub'),
    ).toBeVisible();
    await expect(
      page
        .getByRole('link', {
      ...
      'href',
      'https://github.com/suitenumerique/hub/blob/main/LICENSE',
    );
    await expect(
      h2.getByText('LaSuite Hub, your gateway to the collaborative suite.'),
    ).toBeVisible();
    await expect(
      page.getByText(
  ...
  test('checks all the elements are visible with dsfr theme', async ({
    ...
    await expect(
      page.getByText(
        'LaSuite Hub is already available, log in to access La Suite collaborative tools.',
      ),
    ).toBeVisible();
    await expect(
  ...
  test('it checks the homepage feature flag', async ({ page }) => {
    ...
    // Keyclock login page
    await expect(
      page.locator(`${process.env.SIGN_IN_EL_LOGIN_PAGE}`).getByText('hub'),
    ).toBeVisible();
  });
});
