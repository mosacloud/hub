...
test.describe('Header', () => {
  test('checks all the elements are visible', async ({ page }) => {
    ...
    await expect(header.locator('h1').getByText('LaSuite Hub')).toHaveCSS(
      'font-family',
      /Roboto/i,
    );
  ...
  test('checks all the elements are visible with DSFR theme', async ({
    ...
    await expect(header.locator('h1').getByText('LaSuite Hub')).toHaveClass(
      /sr-only/,
    );
  });
  ...
  test('checks a custom waffle', async ({ page }) => {
      ...
      theme_customization: {
        waffle: {
          data: {
            services: [
              {
                name: 'LaSuite Hub E2E Custom 1',
                url: 'https://hub.numerique.gouv.fr/',
                maturity: 'stable',
                logo: 'https://lasuite.numerique.gouv.fr/assets/products/docs.svg',
              },
              {
                name: 'LaSuite Hub E2E Custom 2',
                url: 'https://hub.numerique.gouv.fr/',
                maturity: 'stable',
                logo: 'https://lasuite.numerique.gouv.fr/assets/products/docs.svg',
              },
    ...
    await expect(
      page.getByRole('link', { name: 'LaSuite Hub E2E Custom 1' }),
    ).toBeVisible();
    await expect(
      page.getByRole('link', { name: 'LaSuite Hub E2E Custom 2' }),
    ).toBeVisible();
  });
