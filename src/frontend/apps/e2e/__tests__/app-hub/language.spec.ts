...
test.describe('Language', () => {
  ...
  test('it checks theme_customization.translations config', async ({
    ...
    await expect(page.getByText('My Custom Hub')).toBeAttached();
  });

  test('checks language switching', async ({ page }) => {
    const header = page.locator('header').first();
    const languagePicker = header.locator('.--hub--language-picker-text');
    ...
    await expect(headerTitle).toContainText('LaSuite Hub');
